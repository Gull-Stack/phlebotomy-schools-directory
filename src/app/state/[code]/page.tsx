...
import { schools, getSchoolsByState, stateNames, getAllStates } from "@/data/schools";

export function generateStaticParams() {
  return getAllStates().map(code => ({ code: code.toLowerCase() }));
}
...
export default async function StatePage({ params }: { params: Promise<{ code: string }> }) {
  ...
  const stateSchools = getSchoolsByState(stateCode);
  ...
  return (
    <main className="min-h-screen bg-gray-950 text-white">
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-2">
                    {school.accreditation.map(acc => (
                      <span key={acc} className="text-xs bg-green-900/50 text-green-400 px-2 py-1 rounded">
                        {acc}
                      </span>
                    ))}
      ...
      <section className="py-12 px-6 bg-gray-900/50">
        <div className="max-w-6xl mx-auto">
          <h2 className="text-2xl font-bold mb-6">
            🌐 Online Programs Available in {stateName}
          </h2>
          <p className="text-gray-400 mb-6">
            These accredited online programs accept students from {stateName}
          </p>
      ...
      <section className="py-12 px-6">
        <div className="max-w-4xl mx-auto">
          <h2 className="text-2xl font-bold mb-6">
            Phlebotomy Requirements in {stateName}
          </h2>
          <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-6">
            <p className="text-gray-300 mb-4">
              {stateCode === 'CA' || stateCode === 'NV' || stateCode === 'LA' || stateCode === 'WA' ? (
                <>
                  <strong className="text-yellow-500">⚠️ {stateName} requires phlebotomist licensure.</strong>
                  {" "}You must complete an approved training program and pass a certification exam to work as a phlebotomist in this state.
                </>
              ) : (
                <>
            ...
            <p className="text-gray-400">
              Average phlebotomist salary in {stateName}: <strong className="text-green-500">$35,000 - $45,000/year</strong>
            </p>
          </div>
        </div>
      </section>
